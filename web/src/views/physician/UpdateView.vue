<template>
  <ElForm
      ...
      <ElFormItem prop="username" label="账号">
        <ElInput v-model="model.username" type="text" placeholder="账号" :disabled="true" />
      </ElFormItem>
...
const fetch = (id) => {
    request.get(`/physician/${id}`).then((response) => {
        if (response.data.status === true) {
            Object.assign(model, response.data.payload.physician)
        }
    })
}
...
const save = (e) => {
  ...
  form.value
    ...
    .then((result) => {
        if (result === true) {
          request.post('/physician/update', { ...model }).then((response) => {
              if (response.data.status === true) {
