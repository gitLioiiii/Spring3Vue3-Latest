...
const fetch = (id) => {
    request.get(`/position/${id}`).then((response) => {
        if (response.data.status === true) {
            Object.assign(model, response.data.payload.position)
        }
    })
}
...
const save = (e) => {
    ...
    form.value
        ...
        .then((result) => {
            if (result === true) {
                request.post('/position/update', { ...model }).then((response) => {
                    if (response.data.status === true) {
                        ElMessage.success('保存成功！')
                        router.push({ name: 'position_index' })
                    } else {
